...
def integrate_data(stock_price_df, news_df, stock_price_csv_file, sentiment_key_name):
    ...
    stock_price_df_copy['Date'] = pd.to_datetime(stock_price_df_copy['Date'], format=r'%Y%m%d')
    ...
    # 4. 合并数据
    merged_df = pd.merge(stock_price_df_copy,  news_df_copy, on='Date', how='left')
    merged_df['Sentiment_textblob'] = (merged_df['Sentiment_textblob']+1)/2
    # 将NaN替换为3
    merged_df[sentiment_key_name].fillna(0.5, inplace=True)

    # df_cleaned = merged_df.dropna(subset=['News_flag'])

    # 过滤掉sentiment列为0的行
    df_cleaned = merged_df#df_cleaned[df_cleaned[sentiment_key_name] != 0]
    # if sentiment_key_name == "Sentiment_gpt":
    #     df_cleaned['Scaled_sentiment'] = df_cleaned[sentiment_key_name].apply(lambda x: (x - 0.9999) / 4)
    # elif sentiment_key_name == "Sentiment_textblob":

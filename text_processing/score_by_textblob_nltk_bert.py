...
def get_sentiment_textblob(symbol, *texts):
    # 构建文本内容
    ...
    #     max_tokens=50,
    # )
    response = TextBlob(text_content).sentiment
...
def from_csv_get_sentiment(df, symbol, saving_path, batch_size=4):
    ...
    for i in range(0, len(df), batch_size):
        ...
        sentiment = get_sentiment_textblob(symbol, *texts)
        df.loc[i, 'Sentiment_textblob'] = sentiment
        # for k, sentiment in enumerate(sentiments):
        #     if i + k < len(df):
        #         df.loc[i + k, 'Sentiment_gpt'] = sentiment
        df_filtered = df[["Date", "Sentiment_textblob"]].copy()
        df_filtered.sort_values(by='Sentiment_textblob', ascending=False, na_position='last', inplace=True)
        df_filtered.to_csv(saving_path, index=False)
    return df


def reproduce(folder_path, saving_path, batch_size):
    name ='SOSHIANEST_NEWS'
    print(name)
    a = time.time()
...
if __name__ == "__main__":
    ...
    reproduce(news_folder_path, news_saving_path, batch_size=5)

...
def main():
    ...
    ####################################################################################
    # Load dataset
    train_loader, test_loader, train_loader_actual, test_loader_actual, input_dim,output_dim, cols, target_index= load_data(dataset_name, preprocess_type, seq_len, pred_len,batch_size, normalization)
    #####################################################################################
    # Explore data
    Explore_data(eda, train_loader, test_loader, preprocess_type, cols, dataset_name)
    ##########3##############################################################################
    # Load model
    model, criterion, optimizer = load_model(model_type, input_dim,output_dim, seq_len, pred_len, lr)
    ...
    # Train model
    if phase == 'train':
        train_losses, val_losses, model = train_model(model, train_loader, test_loader, criterion, optimizer, chkpnt_path, epochs=epoch, device='cpu', load_checkpoint=False)
    ...
    evaluate_model(model, test_loader_actual, cols, 
                   train_losses, val_losses, pred_len, normalization,
